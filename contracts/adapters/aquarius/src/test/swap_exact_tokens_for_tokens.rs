use super::aquarius_adapter_contract::AdapterError as AdapterErrorDeployer;
use crate::test::aqua_setup::create_token_contract;
use crate::test::AquaAggregatorAdapterTest;
use adapter::AdapterError;
use soroban_sdk::testutils::Address as _;
use soroban_sdk::{token::TokenClient, vec, Address, BytesN, FromVal, Symbol, Vec, U256};
...
fn swap_exact_tokens_for_tokens_amount_in_negative() {
    ...
    assert_eq!(result, Err(Ok(AdapterErrorDeployer::NegativeNotAllowed)));
}
...
fn swap_exact_tokens_for_tokens_missing_hash() {
    ...
    assert_eq!(result, Err(Ok(AdapterErrorDeployer::MissingPoolHashes)));
}
...
fn swap_exact_tokens_for_tokens_constant_product_pool_1_hop() {
    ...
    assert_eq!(
        Symbol::from_val(
            &test.env,
            &pool_info.get(Symbol::new(&test.env, "pool_type")).unwrap()
        ),
        Symbol::new(&test.env, "constant_product")
    );
    ...
    let path: Vec<Address> = vec![&test.env, token1.address.clone(), token2.address.clone()];
...
fn swap_exact_tokens_for_tokens_constant_product_pool_2_hops() {
    ...
    // vec pool hash
    let bytes_vec: Vec<BytesN<32>> = vec![&test.env, pool_index1.clone(), pool_index2.clone()];

    let executed_amounts = test
        .adapter_client
        .mock_all_auths()
