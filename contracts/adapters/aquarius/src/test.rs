...
use aqua_setup::{AquaRouter, AquaTest, TokenClient};
...
fn create_aqua_adapter<'a>(e: &Env) -> AquariusAdapterClient<'a> {
    AquariusAdapterClient::new(e, &e.register(AquariusAdapter, {}))
}

pub mod aqua_adapter_contract {
    soroban_sdk::contractimport!(
        file = "../../target/wasm32-unknown-unknown/release/aquarius_adapter.optimized.wasm"
    );
...
pub struct AquaAggregatorAdapterTest<'a> {
    ...
    adapter_client_not_initialized: AquariusAdapterClient<'a>,
    router: AquaRouter<'a>,
...
impl<'a> AquaAggregatorAdapterTest<'a> {
    fn setup() -> Self {
        ...
        let wasm_hash = test
            .env
            .deployer()
            .upload_contract_wasm(aqua_adapter_contract::WASM);
        let deployer_client = create_deployer(&test.env);

        let adapter_client_not_initialized = create_aqua_adapter(&test.env);
        ...
        let init_fn_args: Vec<Val> =
            (protocol_id.clone(), protocol_address.clone()).into_val(&test.env);
